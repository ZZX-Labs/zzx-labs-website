// /portfolio/technology/web/script.js
...
(() => {
  ...
  const SAMPLE_SIZE = 12; // how many to show per load (adjust as you like)
  ...
  // Try to fetch card.html for richer content
  ...
  async function render(items) {
    ...
    if (!items.length) {
      LIST.appendChild(el('p', 'loading', 'No web portfolio items yet.'));
      return;
    }
    ...
    for (const it of sample) {
      ...
      // Progressive enhancement: inject card.html if it exists
      ...
      if (html) {
        ...
        // Only append safe nodes (strip scripts)
