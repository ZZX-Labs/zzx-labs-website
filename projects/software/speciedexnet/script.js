// SpeciedexNet page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "SpeciedexNet preview";
  ...
  try {
    ...
    // buttons
    addBtn("Open Relay", m.href || "/projects/software/speciedexnet/");
    addBtn("GitHub", m.github_url, "ghost");
    addBtn("Docs", m.docs_url, "ghost");
    addBtn("API", m.api_url, "ghost");
    addBtn("Metrics", m.metrics_url, "ghost");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "SpeciedexNet"));
