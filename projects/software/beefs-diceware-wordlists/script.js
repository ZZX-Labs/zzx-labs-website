// beefs-diceware-wordlists page boot — loads ./manifest.json and renders everything
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image image-zoom';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'beefs-diceware-wordlists')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load beefs-diceware-wordlists manifest: ${e.message}`;
  }
