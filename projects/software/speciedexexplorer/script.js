// SpeciedexExplorer page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const installEl = $("#install-snippet");
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "SpeciedexExplorer preview";
  ...
  try {
    ...
    addBtn("Open Explorer", m.href || "/projects/software/speciedexexplorer/");
    addBtn("GitHub", m.github_url, "ghost");
    addBtn("Docs", m.docs_url, "ghost");
    addBtn("API", m.api_url, "ghost");
    addBtn("Hugging Face", m.huggingface_url, "ghost");
    addBtn("Demo", m.demo_url, "ghost");

    // badges: state + latest version
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "SpeciedexExplorer"));
      galHintEl.textContent = "";
    }

    // install snippet override if provided
    if (m.install && installEl) installEl.textContent = m.install;
