// bitcasino page boot — loads ./manifest.json and renders everything
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'bitcasino')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load bitcasino manifest: ${e.message}`;
  }
