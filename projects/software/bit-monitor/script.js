// bit-monitor page boot — loads ./manifest.json and renders everything
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'bit-monitor')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load bit-monitor manifest: ${e.message}`;
  }
