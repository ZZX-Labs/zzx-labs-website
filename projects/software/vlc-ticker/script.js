// VLC-Ticker page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "VLC-Ticker screenshot";
  ...
  try {
    ...
    // buttons (only render when provided)
    addBtn("Open", m.href || "/projects/software/vlc-ticker/");
    addBtn("GitHub", m.github_url, "ghost");
    addBtn("Docs", m.docs_url, "ghost");
    addBtn("Download", m.download_url, "ghost");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "VLC-Ticker"));
