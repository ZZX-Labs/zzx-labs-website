// CyberChefKit page loader — hydrates from ./manifest.json
(async function () {
  ...
  try {
    ...
    if (m.spec)   meta.push(`<li><strong>Spec:</strong> <a href="${attr(m.spec)}" target="_blank" rel="noopener noreferrer">${esc(m.spec)}</a></li>`);
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'CyberChefKit')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load CyberChefKit manifest: ${e.message}`;
  }
