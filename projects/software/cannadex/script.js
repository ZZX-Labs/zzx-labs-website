// Cannadex page loader — hydrates from ./manifest.json
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'Cannadex')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load Cannadex manifest: ${e.message}`;
  }
