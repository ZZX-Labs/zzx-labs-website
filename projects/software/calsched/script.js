// CalSched page loader — hydrates from ./manifest.json
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'CalSched')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load CalSched manifest: ${e.message}`;
  }
