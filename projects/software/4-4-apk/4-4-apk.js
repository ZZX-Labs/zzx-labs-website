(() => {
  ...
  const STORAGE_SETTINGS = "zzx-4-4-apk-settings-v1";
  const STORAGE_STATS = "zzx-4-4-apk-stats-v1";
  ...
  function exposeApi() {
    window.FourFourAPK = Object.freeze({
      version: "1.0.0-apk-web",
      start: startSession,
      pause: pauseSession,
  ...
  // ---------------------------------------------------------------
  // APK-safe-loader integration.
  // APK bytes remain inert and are never executed by this page.
  // ---------------------------------------------------------------

  function setApkBadge(id, stateName) {
    const badge = $(id);
    if (!badge) return;
    badge.className = `apk-badge ${stateName || ""}`.trim();
  }

  function setApkState(text, className = "ok") {
    const status = $("status-apk");
    const stateEl = $("apk-state");

    if (status) {
      status.textContent = `APK: ${text}`;
      status.className = `runtime-badge ${className}`;
    }

    if (stateEl) stateEl.textContent = text;
  }

  function renderApkEntries(entries) {
    ...
    for (const entry of entries.slice(0, 1000)) {
      ...
      row.className = "apk-entry-row";

      const name = document.createElement("code");
      name.textContent = entry.name;

      const size = document.createElement("span");
      size.textContent = window.APKInspector.formatBytes(entry.uncompressedSize);
    ...
    if (entries.length > 1000) {
      const row = document.createElement("div");
      row.className = "apk-entry-row";
      row.innerHTML = `<code>… ${entries.length - 1000} additional entries omitted from display</code><span>—</span>`;
      list.appendChild(row);
    }
  }

  function renderApkResult(result) {
    $("apk-size").textContent = result.sizeFormatted;
    $("apk-entry-count").textContent = String(result.entryCount);
    $("apk-dex-count").textContent = String(result.dexFiles.length);
    $("apk-sha256").value = result.sha256;

    setApkBadge("apk-badge-zip", result.zip ? "ok" : "no");
    setApkBadge("apk-badge-manifest", result.androidManifest ? "ok" : "no");
    setApkBadge("apk-badge-dex", result.dexFiles.length ? "ok" : "no");
    setApkBadge("apk-badge-resources", result.resourcesArsc ? "ok" : "partial");
    setApkBadge(
      "apk-badge-signature",
      result.hasApkSigningBlock || result.legacySignatureFiles.length ? "ok" : "partial"
    );

    setApkState(result.likelyApk ? "VALID APK" : "ZIP / NONSTANDARD APK",
      result.likelyApk ? "ok" : "partial");

    renderApkEntries(result.entries);
    ...
    $("apk-output").textContent = JSON.stringify({
      ...
      zipEntries: result.entryCount,
      androidManifest: result.androidManifest,
      resourcesArsc: result.resourcesArsc,
      dexFiles: result.dexFiles,
      nativeLibraryCount: result.nativeLibs.length,
      legacySignatureFiles: result.legacySignatureFiles,
      apkSigningBlockDetected: result.hasApkSigningBlock,
      likelyApk: result.likelyApk
    }, null, 2);

    window.ZZXHooks?.emit("4-4-apk:inspected", {
      sha256: result.sha256,
  ...
  async function inspectLocalApk(file) {
    if (!window.APKInspector) throw new Error("APK inspector module is unavailable.");

    setApkState("READING", "partial");
    $("apk-output").textContent = `Reading ${file.name}…`;

    try {
      const result = await window.APKInspector.inspectFile(file);
      renderApkResult(result);
      return result;
    } catch (error) {
      setApkState("ERROR", "no");
      $("apk-output").textContent = `ERROR: ${error.message}`;
      throw error;
    }
  }

  async function inspectHostedApk() {
    if (!window.APKInspector) throw new Error("APK inspector module is unavailable.");

    const url = "./4-4-apk.apk";
    setApkState("FETCHING", "partial");
    $("apk-output").textContent = `Fetching ${url} as inert package data…`;

    try {
      const result = await window.APKInspector.inspectUrl(url);
      renderApkResult(result);
      return result;
    } catch (error) {
      setApkState("NOT FOUND", "partial");
      $("apk-output").textContent =
        `Hosted APK could not be inspected.\n${error.message}\n\n` +
        `Place the existing APK at ${url} or select it locally.`;
      return null;
    }
  }

  function bindApkLoader() {
    bind("load-hosted-apk", "click", inspectHostedApk);

    bind("apk-file", "change", async () => {
      const file = $("apk-file").files?.[0];
      if (file) await inspectLocalApk(file);
    });

    const dropzone = $("apk-dropzone");
    if (!dropzone) return;
    ...
    dropzone.addEventListener("drop", async (event) => {
      ...
      if (!file.name.toLowerCase().endsWith(".apk")) {
        setApkState("REJECTED", "no");
        $("apk-output").textContent = "Dropped file does not use the .apk extension.";
        return;
      }

      try {
        await inspectLocalApk(file);
      } catch (error) {
        console.error(error);
      }
    });
  ...
  // Add APK functions to a dedicated secondary API without changing
  // the frozen breathing-engine API object.
  window.FourFourAPKPackage = Object.freeze({
    inspectFile: inspectLocalApk,
    inspectHosted: inspectHostedApk
  });

  window.ZZXHooks?.emit("4-4-apk:ready", {
    version: "1.0.0-apk-web",
    hostedApk: "./4-4-apk.apk"
  });
