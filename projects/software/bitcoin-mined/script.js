// bitcoin-mined page boot — loads ./manifest.json and renders
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'bitcoin-mined')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load bitcoin-mined manifest: ${e.message}`;
  }
