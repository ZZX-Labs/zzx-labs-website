(()=>{"use strict";
const $=id=>document.getElementById(id),FX=FreedomXCore,REGION="US",MODE="DEX",PAIR="BTC/USD";
...
function applyAmm(){
 amm=new DEXResearchEngine(Math.max(.0001,+$("reserve-btc").value||100),Math.max(.01,+$("reserve-quote").value||8400000),Math.max(0,+$("amm-fee").value||30));
...
function genPeerResearch(){
 ...
 peerOffers=peerResearch.offers(+$("mid").value||84000,Math.max(4,+$("offer-count").value||20),Math.max(1,+$("spread-bps").value||25),.10);
...
$("export-state").onclick=()=>{const doc={schema:"zzx.freedomx.dex.v3.1",project:"freedomkobradex",region:REGION,mode:MODE,pair:PAIR,bitcoinOnly:true,noncustodial:true,livePeerMarket:{offers:market.offers},lastPeerRoute:lastRoute,originalAmmResearch:{pool:amm?{reserveBTC:amm.x,reserveQuote:amm.y,feeBps:amm.feeBps}:null,lastSwap:lastAmmSwap,lastRoute:lastAmmRoute,lastLiquidity:lastAmmLiquidity},peerResearch:{offers:peerOffers,lastMonteCarlo:lastMC,lastConcentration},settlements:settlements.map(s=>({id:s.id,current:s.current(),trade:s.trade})),quorum:quorum.status(),events};const t=JSON.stringify(doc,null,2);FX.download(t,"freedomkobradex-state.json");$("export-output").textContent=t};
...
window.ZZXHooks?.emit("freedomkobradex:ready",{version:"0.3.1-alpha-web"});
})();
