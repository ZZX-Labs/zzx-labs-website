// MantraBox page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "MantraBox screenshot";
  ...
  try {
    ...
    addBtn("Open", m.href || "/projects/software/mantrabox/");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "MantraBox"));
