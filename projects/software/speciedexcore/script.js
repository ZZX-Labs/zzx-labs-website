// SpeciedexCore page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "SpeciedexCore preview";
  ...
  try {
    ...
    addBtn("Open Project", m.href || "/projects/software/speciedexcore/");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "SpeciedexCore"));
