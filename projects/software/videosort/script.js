// VideoSort page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "VideoSort screenshot";
  ...
  try {
    ...
    // buttons
    addBtn("Open", m.href || "/projects/software/videosort/");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "VideoSort"));
