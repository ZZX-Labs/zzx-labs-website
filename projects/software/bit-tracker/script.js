// bit-tracker page boot — loads ./manifest.json and renders everything
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'bit-tracker')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load bit-tracker manifest: ${e.message}`;
  }
