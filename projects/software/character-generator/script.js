// Character Generator page loader — hydrates from ./manifest.json
(async function () {
  ...
  try {
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'Character Generator')} image" loading="lazy" decoding="async" />`;
    ...
    console.error(e);
    descEl.textContent = `Failed to load Character Generator manifest: ${e.message}`;
  }
