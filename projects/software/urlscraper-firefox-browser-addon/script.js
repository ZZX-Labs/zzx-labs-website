// URLScraper page boot: fetch ./manifest.json → hydrate hero, buttons, badges, gallery.
(async function () {
  ...
  const addImg = (src, alt) => {
    ...
    img.decoding = "async";
    img.src = src;
    img.alt = alt || "URLScraper screenshot";
  ...
  try {
    ...
    // buttons
    addBtn("Open", m.href || "/projects/software/urlscraper-firefox-browser-addon/");
    addBtn("GitHub", m.github_url, "ghost");
    addBtn("Docs", m.docs_url, "ghost");
    addBtn("AMO Add-on", m.amo_url, "ghost");
    ...
    if (!imgs.length) {
      ...
      imgs.forEach((src) => addImg(src, m.title || "URLScraper"));
