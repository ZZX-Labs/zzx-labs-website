// Project page loader — hydrates from ./manifest.json, sets buttons, lists, and images
(async function () {
  ...
  try {
    ...
    if (m.algorithms)   meta.push(li(`Algorithms:`, esc(m.algorithms.join(', '))));
    if (m.outputs)      meta.push(li(`Outputs:`, esc(m.outputs.join(', '))));
    ...
    imgs.forEach(src => {
      ...
      f.className = 'image';
      f.innerHTML = `<img src="${attr(src)}" alt="${esc(m.title || 'HashPig')}" loading="lazy" decoding="async" />`;
