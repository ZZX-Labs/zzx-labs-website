(function(){
  ...
  const PAGE_KEY="zzx.widget.nodes-by-county.page-size.v5";
  ...
  async function ensureModules(core){
    ...
    await loadScript(
      ...
      ()=>Number(W.ZZXNodesByCountyModel?.__version||0)>=1,
      "ZZXNodesByCountyModel"
    );
  }

  function pageSize(root){
    const n=Number(q(root,"[data-nbco-page-size]")?.value);
  ...
  function renderTable(root,state){
    ...
    if(!slice.length){
      ...
      slice.forEach((item,index)=>{
        ...
        values.forEach((value,i)=>{
          ...
          if(i>=4)cell.classList.add("nodes-by-county__num");
    ...
    set(
      root,
      "[data-nbco-page]",
      `Page ${state.page+1} / ${pages} · ${rows.length.toLocaleString()} count${rows.length===1?"y/admin-2":"ies/admin-2 rows"}`
    );

    const prev=q(root,"[data-nbco-prev]");
    const next=q(root,"[data-nbco-next]");
  ...
  function render(root,state){
    ...
    set(
      root,
      "[data-nbco-sub]",
      `${m.countyCount.toLocaleString()} county/admin-2 row${m.countyCount===1?"":"s"} · ${pct(m.coverage)} reachable coverage`
    );
    set(root,"[data-nbco-county-count]",m.countyCount.toLocaleString());
    set(root,"[data-nbco-top-county]",m.top?.label||"—");
    ...
    set(root,"[data-nbco-known]",`${integer(m.geolocatedTotal)} · ${pct(m.coverage)}`);
    set(
      root,
      "[data-nbco-unknown]",
    ...
    set(root,"[data-nbco-network-total]",integer(m.reachable));
    ...
    set(
      root,
      "[data-nbco-meta]",
      "ZZXBitnodes v5 shared snapshot · county+region+country keys · zero per-widget node API calls"
    );
    ...
    W.ZZXNodesByCounty=Object.freeze({
      schema:"zzx-nodes-by-county-export-v1",
  ...
  async function refresh(root,state,force=false){
    ...
    try{
      ...
      state.model=W.ZZXNodesByCountyModel.build(state.result.snapshot);
      state.page=0;
      render(root,state);
    }catch(error){
  ...
  async function boot(root,core){
    ...
    try{
      ...
      const size=q(root,"[data-nbco-page-size]");
      const saved=Number(safeGet(PAGE_KEY));
      if(size&&[5,10,20,50].includes(saved))size.value=String(saved);

      q(root,"[data-nbco-prev]")?.addEventListener("click",()=>{
        state.page=Math.max(0,state.page-1);
        renderTable(root,state);
      },options);

      q(root,"[data-nbco-next]")?.addEventListener("click",()=>{
        state.page+=1;
        renderTable(root,state);
      },options);

      q(root,"[data-nbco-search]")?.addEventListener("input",()=>{
