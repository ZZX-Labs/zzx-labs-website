(function(){
  ...
  if(W.ZZXBitcoinTickerNationalDebts?.__version>=1)return;
  ...
  function mount(root,state){
    if(root.__zzxTickerNationalDebtsMounted)return;
    root.__zzxTickerNationalDebtsMounted=true;
    root.querySelector("[data-debt-country]")?.addEventListener("change",event=>{
      try{localStorage.setItem(W.ZZXBitcoinTickerConstants.storage.debtCountry,event.currentTarget.value)}catch(_){}
      if(state.debts)render(root,state.debts,state.chainHeight,state.issuedSats);
    });
  }

  async function update(root,state,height,issued,force=false){
    if(force||!state.debts){
      state.debts=await load(force);
      populate(root,state.debts);
    }
    render(root,state.debts,height,issued);
    return state.debts;
  }
  ...
  W.ZZXBitcoinTickerNationalDebts=api;
  W.ZZXBitcoinTickerDebts=api;
})();
