(function(){
  ...
  function navigation(sourcePages){
    ...
    return [
      ...
      ...pages.map(page=>({
        ...
        label:page.label,
        panel:"references",
        referencePage:page.id
      }))
    ];
  }

  function mount(root,state){
    if(root.__zzxTickerPurchasingPowerMounted)return;
    root.__zzxTickerPurchasingPowerMounted=true;
    ...
    q(root,"[data-reference-search]")?.addEventListener("input",()=>{
      ...
      searchTimer=W.setTimeout(()=>{
        if(state.references&&state.selection){
          render(root,state,state.selection.priceUsd);
        }
      },120);
    });

    q(root,"[data-reference-page-select]")?.addEventListener("change",event=>{
      if(!state.references||!state.selection)return;
      setPage(root,state,event.currentTarget.value,state.selection.priceUsd);
    });
  }

  async function update(root,state,btcUsd,force=false){
    if(force||!state.references){
      state.references=await load(force);
      populatePages(root,state,state.references,btcUsd);
    }
    render(root,state,btcUsd);
  ...
  W.ZZXBitcoinTickerReferences=api;
})();
