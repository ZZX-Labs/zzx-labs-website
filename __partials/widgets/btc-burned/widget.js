...
(function () {
  ...
  window.ZZXWidgets.register(ID, {
    ...
    async load() {
      // Local dataset in widget folder
      const ctx = this._ctx;
      const url = ctx?.urlFor
        ? ctx.urlFor("/__partials/widgets/btc-burned/btc-burned.json")
        : "/__partials/widgets/btc-burned/btc-burned.json";

      try {
        // Prefer unified runtime fetch
        if (ctx?.fetchJSON) {
    ...
    render() {
      ...
      list.innerHTML = slice.map((it) => {
        ...
        const right = src
          ...
          : `${btc} BTC`;
        return `<div class="row"><span class="k">burn</span><span class="v">${label}${when} · ${right}</span></div>`;
      }).join("");

      if (!slice.length) {
        list.innerHTML = `<div class="row"><span class="k">burn</span><span class="v">no data</span></div>`;
      }
