  ...
  window.ZZXWidgets.register(ID, {
    ...
    async update(force = false) {
      ...
      try {
        ...
        const batch = [
          REPOS[idx],
          REPOS[(idx + 1) % REPOS.length]
        ];

        const items = [];
        for (const repo of batch) {
          try {
            items.push(await latestCommit(repo));
          } catch {}
        }
        ...
        host.innerHTML = items.map(it =>
          `<div style="margin:.2rem 0;">
            <span style="color:#e6a42b;font-family:AdultSwimFont,IBMPlexMono,monospace;">
              ${it.repo.split("/")[1]}
            </span>
            <a href="${it.url}" target="_blank" rel="noopener noreferrer"
               style="color:#c0d674;text-decoration:none;">
              ${it.msg}
            </a>
          </div>`
        ).join("");
