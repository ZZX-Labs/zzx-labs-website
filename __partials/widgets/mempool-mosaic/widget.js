// Mempool Mosaic v3.0.0 — exact square mosaic + persistent transaction readers.
(function(){
  "use strict";
  ...
  if(W.__ZZX_MEMPOOL_MOSAIC_WIDGET_V20__)return;
  W.__ZZX_MEMPOOL_MOSAIC_WIDGET_V20__=true;

  const MODULES=[
    ["ZZXMempoolMosaicSources","js/sources.js",3],
    ["ZZXMempoolMosaicFetch","js/fetch.js",1],
    ["ZZXMempoolMosaicProvider","js/provider.js",1],
    ["ZZXMempoolMosaicLive","js/live.js",2],
    ["ZZXMempoolMosaicAnalyzer","js/analyzer.js",3],
    ["ZZXMempoolMosaicModel","js/model.js",3],
    ["ZZXMempoolMosaicScaler","js/scaler.js",4],
    ["ZZXMempoolMosaicSorter","js/sorter.js",1],
    ["ZZXMempoolMosaicPacker","js/packer.js",5],
    ["ZZXMempoolMosaicLayout","js/layout.js",5],
    ["ZZXMempoolMosaicThemes","js/themes.js",3],
    ["ZZXMempoolMosaicRenderer","js/renderer.js",5],
    ["ZZXMempoolMosaicAnimation","js/animation.js",1],
    ["ZZXMempoolMosaicReaderStore","js/reader-store.js",1],
    ["ZZXMempoolMosaicTxFetcher","js/txfetcher.js",2],
  ...
  function loadScript(src,key,minimumVersion){
    ...
    const selector=`script[data-mm-module="${tagKey}"][data-mm-version="${minimumVersion}"]`;
  ...
  async function mount(root,mountedCore=null){
    ...
    const updated=root.querySelector("[data-mm-updated]");

    if(!canvas||!stage||!summary||!sub)throw new Error("Mempool Mosaic markup is incomplete");
    ...
    let colorMode=storageGet("zzx.mempoolMosaic.color","fee-vbytes");
    let sortMode=storageGet("zzx.mempoolMosaic.sort","priority");
    let themeId=storageGet("zzx.mempoolMosaic.theme","zzx-default");
    let shuffleSeed=Number(storageGet("zzx.mempoolMosaic.shuffleSeed",Date.now()))||Date.now();
    let aborter=null;
    let live=null;
    ...
    if(!W.ZZXMempoolMosaicThemes.list().some(theme=>theme.id===themeId))themeId="zzx-default";

    const sourceConfig=()=>model?.cfg||W.ZZXMempoolMosaicSources.get(runtimeCore);
    const readerLimit=()=>Math.max(1,Number(sourceConfig().readerLimit)||64);

    function populateThemes(){
      const select=root.querySelector("[data-mm-theme-select]");
      if(!select)return;
    ...
    function renderLegend(){
      const host=root.querySelector("[data-mm-legend]");
      if(!host)return;
    ...
    function applyTheme(){
      W.ZZXMempoolMosaicThemes.apply(root,themeId);
      const select=root.querySelector("[data-mm-theme-select]");
    ...
    function setActive(){
      ...
      const color=root.querySelector("[data-mm-color]");
      const sort=root.querySelector("[data-mm-sort]");
    ...
    function redraw(){
      if(!layout)return;
      W.ZZXMempoolMosaicRenderer.draw(canvas,layout,{
        fromLayout:null,
        progress:1,
        selectedTxid,
        hoverTxid,
    ...
    function renderStats(){
      ...
      sub.textContent=`${sourceLabel} · square area ${scaleLabel} · chroma ${colorLabel} · ${sortMode} topology`;
      ...
      const leafNode=root.querySelector("[data-mm-leaves]");
      const valueCoverage=root.querySelector("[data-mm-value-coverage]");
      const coverageValue=root.querySelector("[data-mm-coverage-value]");
      const depth=root.querySelector("[data-mm-depth]");
      if(leafNode)leafNode.textContent=`${fmtInt(layout.leafCount)} tesserae${layout.fragmentCount?` · ${layout.fragmentCount} dust continuation`:""}`;
      ...
      if(coverage)coverage.textContent=`${fmtInt(txs)} real candidate TX · ${fmtInt(layout.leafCount)} square tesserae · ${(layout.coverage*100).toFixed(3)}% mosaic coverage · no empty cells`;
      ...
      if(meta)meta.textContent=`square-only mosaic · ${W.ZZXMempoolMosaicThemes.get(themeId).name} · ${scaleLabel} area · ${colorLabel} color · persistent readers local to this browser`;
    ...
    function layoutNow(animate=true){
      ...
      const next=W.ZZXMempoolMosaicLayout.build(model,{scaleMode,sortMode,seed:shuffleSeed});
      priorLayout=layout;
      layout=next;
      animationCancel?.();
      ...
      if(!animate||reduced||!priorLayout){
        ...
        animationCancel=W.ZZXMempoolMosaicAnimation.run(340,progress=>{
          W.ZZXMempoolMosaicRenderer.draw(canvas,layout,{
    ...
    function pointerTile(event){
      ...
      return W.ZZXMempoolMosaicLayout.hit(
        layout,
    ...
    async function refreshReaders(){
      const records=await W.ZZXMempoolMosaicReaderStore.list(readerLimit());
      if(!destroyed)W.ZZXMempoolMosaicInspector.renderReaderList(root,records,selectedTxid);
      return records;
    }
    ...
    async function openStored(txid){
      ...
      selectedTxid=txid;
      await W.ZZXMempoolMosaicReaderStore.put({...record,lastViewedAt:Date.now()},readerLimit());
      if(record.analysis)W.ZZXMempoolMosaicInspector.render(root,record.analysis,{persisted:true});
      else W.ZZXMempoolMosaicInspector.renderSummary(root,record);
      redraw();
    ...
    async function inspect(txid,{force=false}={}){
      ...
      if(!W.ZZXMempoolMosaicReaderStore.valid(id))return false;
      const generation=++readGeneration;
      selectedTxid=id;
      redraw();

      const stored=await W.ZZXMempoolMosaicReaderStore.get(id);
      ...
      await W.ZZXMempoolMosaicReaderStore.pin(id,tile,readerLimit());
      W.ZZXMempoolMosaicInspector.loading(root,id);
      await refreshReaders();

      try{
        if(force)W.ZZXMempoolMosaicTxFetcher.forget(id);
        const analysis=await W.ZZXMempoolMosaicTxFetcher.full(runtimeCore,id,{
          force,
          tipHeight:model?.tipHeight,
          priceUsd:model?.priceUsd,
          tile
        });
        await W.ZZXMempoolMosaicReaderStore.save(analysis,tile,readerLimit());

        if(model?.byTxid?.has(id)){
          model=W.ZZXMempoolMosaicModel.mergeDetails(model,[analysis.tx]);
          if(liveSnapshot?.transactions?.length)model=W.ZZXMempoolMosaicModel.mergeLive(model,liveSnapshot);
          layoutNow(true);
        }

        if(!destroyed&&generation===readGeneration&&selectedTxid===id){
          W.ZZXMempoolMosaicInspector.render(root,analysis,{persisted:false});
          redraw();
        }
        await refreshReaders();
        return true;
      }catch(error){
        await W.ZZXMempoolMosaicReaderStore.fail(id,error,readerLimit());
        if(!destroyed&&generation===readGeneration&&selectedTxid===id){
          W.ZZXMempoolMosaicInspector.error(root,id,error?.message||error);
        }
        await refreshReaders();
        return false;
      }
    ...
    async function hydrate(){
      ...
      if(model.liveActive){
        ids=W.ZZXMempoolMosaicModel.pendingCandidateTxids(model,cfg.hydrateBatch);
      }else if(!model.fullFeedActive){
        ids=W.ZZXMempoolMosaicModel.pendingUniverseTxids(model,cfg.fallbackHydrateBatch,fallbackCursor);
        ...
        if(!ids.length&&fallbackCursor>0){
          fallbackCursor=0;
          ids=W.ZZXMempoolMosaicModel.pendingUniverseTxids(model,cfg.fallbackHydrateBatch,0);
        }
      }else{
        ids=W.ZZXMempoolMosaicModel.pendingCandidateTxids(model,cfg.hydrateBatch);
      }
      ...
      const rows=await W.ZZXMempoolMosaicTxFetcher.batch(runtimeCore,ids,{concurrency});
      if(destroyed||paused||!rows.length)return;
      model=W.ZZXMempoolMosaicModel.mergeDetails(model,rows);
      if(liveSnapshot?.transactions?.length)model=W.ZZXMempoolMosaicModel.mergeLive(model,liveSnapshot);
      layoutNow(true);
    ...
    function consumeLive(snapshot){
      ...
      model=W.ZZXMempoolMosaicModel.mergeLive(model,snapshot);
      layoutNow(true);
      W.clearTimeout(hydrateTimer);
    ...
    async function refresh(force=false){
      ...
      try{
        const payload=await W.ZZXMempoolMosaicProvider.load(runtimeCore,{signal:aborter.signal,force});
        if(destroyed)return;
        const fresh=W.ZZXMempoolMosaicModel.build(payload);
        model=liveSnapshot?.transactions?.length
          ? W.ZZXMempoolMosaicModel.mergeLive(fresh,liveSnapshot)
          : fresh;
        ...
        summary.textContent=layout?summary.textContent:"Mempool Mosaic unavailable";
    ...
    root.querySelector("[data-mm-color]")?.addEventListener("change",event=>{
    ...
    root.querySelector("[data-mm-theme-select]")?.addEventListener("change",event=>{
    ...
    root.querySelector("[data-mm-pause]")?.addEventListener("click",event=>{
      paused=!paused;
    ...
    root.querySelector("[data-mm-refresh]")?.addEventListener("click",async()=>{
      paused=false;
      const pause=root.querySelector("[data-mm-pause]");
    ...
    root.querySelector("[data-mm-fullscreen]")?.addEventListener("click",async()=>{
      try{
    ...
    root.querySelector("[data-mm-open-form]")?.addEventListener("submit",event=>{
      event.preventDefault();
      const input=root.querySelector("[data-mm-open-txid]");
      const error=root.querySelector("[data-mm-open-error]");
      const txid=String(input?.value||"").trim().toLowerCase();
      if(!W.ZZXMempoolMosaicReaderStore.valid(txid)){
    ...
    root.querySelector("[data-mm-reader-list]")?.addEventListener("click",async event=>{
      const remove=event.target.closest("[data-mm-reader-remove]");
      if(remove){
        event.stopPropagation();
        const txid=remove.dataset.mmReaderRemove;
        await W.ZZXMempoolMosaicReaderStore.remove(txid);
        if(selectedTxid===txid){selectedTxid="";W.ZZXMempoolMosaicInspector.clear(root);redraw()}
        await refreshReaders();
        return;
      }
      const chip=event.target.closest("[data-mm-reader-open]");
      if(chip)await openStored(chip.dataset.mmReaderOpen);
    },listenerOptions);

    root.querySelector("[data-mm-reader-list]")?.addEventListener("keydown",event=>{
      if((event.key==="Enter"||event.key===" ")&&event.target.matches("[data-mm-reader-open]")){
        event.preventDefault();
        openStored(event.target.dataset.mmReaderOpen);
      }
    },listenerOptions);
    ...
    root.querySelector("[data-mm-refresh-reader]")?.addEventListener("click",()=>{
      if(selectedTxid)inspect(selectedTxid,{force:true});
    },listenerOptions);

    root.querySelector("[data-mm-unpin-reader]")?.addEventListener("click",async()=>{
      if(!selectedTxid)return;
      await W.ZZXMempoolMosaicReaderStore.remove(selectedTxid);
      selectedTxid="";
      W.ZZXMempoolMosaicInspector.clear(root);
      redraw();
      await refreshReaders();
    ...
    root.querySelector("[data-mm-copy-txid]")?.addEventListener("click",async event=>{
      if(!selectedTxid)return;
      try{
    ...
    root.querySelector("[data-mm-export-readers]")?.addEventListener("click",async()=>{
      const data=await W.ZZXMempoolMosaicReaderStore.exportAll();
      downloadJSON(`mempool-mosaic-readers-${new Date().toISOString().replace(/[:.]/g,"-")}.json`,data);
    },listenerOptions);

    root.querySelector("[data-mm-clear-readers]")?.addEventListener("click",async()=>{
      if(typeof W.confirm==="function"&&!W.confirm("Clear every locally persisted Mempool Mosaic transaction reader?"))return;
    ...
    const lastId=W.ZZXMempoolMosaicReaderStore.last();
    ...
    root.__mempoolMosaicDestroy=()=>{
      ...
      resize?.disconnect?.();
      listenerAbort?.abort();
      root.__mempoolMosaicMounted=false;
    };
  }
  ...
  async function bootRoot(root,mountedCore){
    try{
      ...
      if(summary)summary.textContent="Mempool Mosaic failed to initialize";
      if(status){
  ...
  function fallbackBoot(){
    const run=()=>D.querySelectorAll('[data-widget-root="mempool-mosaic"]').forEach(root=>bootRoot(root,core()));
