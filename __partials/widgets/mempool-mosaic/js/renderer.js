(function(){
  "use strict";
  const W=window;
  if(W.ZZXMempoolMosaicRenderer?.__version>=2)return;
  ...
  function draw(canvas,tiles,stats,{animate=true}={}){
    ...
    const pad=3;
    const targets=W.ZZXMempoolMosaicTreemap.layout(tiles,surface.width-pad*2,surface.height-pad*2)
    ...
    const duration=Math.max(0,Number(W.ZZXMempoolMosaicSources?.animationMs)||420);
  ...
  W.ZZXMempoolMosaicRenderer=Object.freeze({__version:2,color,draw,hitTest,reset});
})();
