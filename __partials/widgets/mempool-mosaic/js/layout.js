// v5 — exact gapless square mosaic for every projected-block transaction.
(function(){
  "use strict";

  const W=window;
  if(W.ZZXMempoolMosaicLayout?.__version>=5)return;

  const Scaler=()=>W.ZZXMempoolMosaicScaler;
  const Sorter=()=>W.ZZXMempoolMosaicSorter;
  const Packer=()=>W.ZZXMempoolMosaicPacker;
  const clamp=(n,a,b)=>Math.max(a,Math.min(b,n));

  function empty(scaleMode,sortMode,seed){
    return {
      schema:"zzx-mempool-mosaic-square-layout-v5",
      tiles:[],
      byTxid:new Map(),
      root:null,
      coverage:0,
  ...
  W.ZZXMempoolMosaicLayout=Object.freeze({
    __version:5,
    build,
    hit
  });
