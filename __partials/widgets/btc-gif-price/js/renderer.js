(function () {
  "use strict";
  const W=window;
  if(W.ZZXBTCGifPriceRenderer?.__version>=1)return;
  ...
  function draw(canvas,img,market){
    ...
    const panelH=Math.max(48*dpr,height*.27),y=height-panelH;
    ...
    ctx.fillStyle=gradient;ctx.fillRect(0,y,width,panelH);
    const pad=10*dpr,price=market?.priceText||"BTC —",delta=market?.deltaText||"24h —",detail=market?.detailText||"ZZX Global BPI";
    ctx.textBaseline="alphabetic";
    ctx.font=`${Math.max(16,20*dpr)}px ui-monospace,monospace`;
    ctx.fillStyle="#c0d674";ctx.fillText(price,pad,height-24*dpr);
    ctx.font=`${Math.max(10,11*dpr)}px ui-monospace,monospace`;
    ctx.fillStyle=market?.deltaTone==="down"?"#d67474":market?.deltaTone==="up"?"#c0d674":"#e6a42b";
    const deltaWidth=ctx.measureText(delta).width;
    ctx.fillText(delta,Math.max(pad,width-pad-deltaWidth),height-24*dpr);
    ctx.fillStyle="#e6a42b";ctx.fillText(detail,pad,height-8*dpr);
  }
  W.ZZXBTCGifPriceRenderer=Object.freeze({__version:1,draw,size});
})();
