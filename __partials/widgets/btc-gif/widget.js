(function(){
  ...
  const D=document;
  const ID="btc-gif";
  const AUTO_KEY="zzx.widget.btc-gif.auto.v3";
  const SIGNAL_REFRESH_MS=15000;

  const MODULES=Object.freeze([
    {global:"ZZXBTCGifTelemetry",path:"js/telemetry.js",version:2},
    {global:"ZZXBTCGifConditions",path:"js/conditions.js",version:2},
    {global:"ZZXBTCGifRenderer",path:"js/renderer.js",version:2}
  ]);
  ...
  function poolSize(state){
    ...
    return W.ZZXBTCGifConditions.pool(state.library,id).length;
  }

  function choose(root,state,{force=false}={}){
    if(!state.condition||!state.library)return;
    const id=state.condition.winner?.id||"neutral";
    const item=W.ZZXBTCGifConditions.pick(
      state.library,
      id,
  ...
  async function refreshSignals(root,state,{force=false}={}){
    ...
    try{
      ...
      state.telemetry=await W.ZZXBTCGifTelemetry.load(state.core,{force});
      state.condition=W.ZZXBTCGifConditions.evaluate(state.telemetry,state.policy);
