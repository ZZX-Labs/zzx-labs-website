// __partials/widgets/volume-24h/js/sources.js
// Exchange definitions + normalized hourly candles.
// Registers window.ZZXVolume24Sources.
...
(function () {
  ...
  if (W.ZZXVolume24Sources?.__version >= 1) return;
  ...
  function completedHourly(candles, nowMs) {
    ...
    return normalizeCandles(candles).filter(candle => {
      // All supported endpoints use the candle start time.
      return (candle.t + HOUR_MS) <= now;
    });
  }

  function final48Completed(candles, nowMs) {
    return completedHourly(candles, nowMs).slice(-48);
  }
  ...
  const sources = [
    {
      ...
      normalize(payload) {
        return final48Completed((Array.isArray(payload) ? payload : []).map(row => ({
          t: number(row?.[0]) * 1000,
          l: row?.[1],
          h: row?.[2],
          o: row?.[3],
...

    {
      ...
      normalize(payload) {
        ...
        return final48Completed(rows.map(row => ({
          t: number(row?.[0]) * 1000,
          o: row?.[1],
          h: row?.[2],
          l: row?.[3],
...

    {
      ...
      normalize(payload) {
        ...
        return final48Completed((Array.isArray(rows) ? rows : []).map(row => ({
