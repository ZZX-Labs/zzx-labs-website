...
(function(){
  ...
  const PAGE_KEY="zzx.widget.nodes-by-nation.page-size.v7";
  ...
  async function loadScript(path,test,tag){
    ...
    if(existing){
      ...
      while(!test()&&Date.now()-started<2500)await new Promise(done=>W.setTimeout(done,25));
      if(test())return;
    }
  ...
  async function ensureModules(core){
    ...
    await loadScript(`${base(core)}/js/model.js`,()=>Number(W.ZZXNodesByNationModel?.__version||0)>=7,"ZZXNodesByNationModel");
    await loadScript(`${base(core)}/js/provider.js`,()=>Number(W.ZZXNodesByNationProvider?.__version||0)>=7,"ZZXNodesByNationProvider");
  }
  ...
  function filtered(root,state){
    ...
    return (state.model?.rows||[]).filter(item=>`${item.code} ${item.name}`.toLowerCase().includes(needle));
  }
  ...
  function nationCell(item){
    ...
    name.title=`${item.code||"--"} · ${name.textContent}`;
  ...
  function render(root,state){
    ...
    set(root,"[data-nbn-updated]",Number.isFinite(finite(m.updatedMs))?new Date(m.updatedMs).toLocaleString():"—");
    set(root,"[data-nbn-source]",`${result.source||snapshot.source||"shared ZZX Bitnodes"} · ${result.transport||"shared"}${result.stale?" · stale":""}`);
    set(root,"[data-nbn-meta]",`${m.mode==="per-node"?"per-node canonical geography":"aggregate country distribution"} · flag + ISO-2 + full nation name`);
    ...
    W.ZZXNodesByNation=Object.freeze({
      schema:"zzx-nodes-by-nation-export-v7",
      mode:m.mode,
      rows:Object.freeze(exportRows),
      byNation:Object.freeze(Object.fromEntries(exportRows.map(row=>[
        row.code,
        Object.freeze({country:row.name,code:row.code,flag:row.flag,nodes:row.nodes,share:row.share,geoShare:row.geoShare})
      ]))),
  ...
  async function boot(root,core){
    ...
    try{
      ...
      if(size&&[5,10,20,50].includes(savedSize))size.value=String(savedSize);
      ...
      q(root,"[data-nbn-search]")?.addEventListener("input",()=>{
        if(state.searchTimer)W.clearTimeout(state.searchTimer);
        state.searchTimer=W.setTimeout(()=>{state.page=0;renderTable(root,state);},120);
      },options);
