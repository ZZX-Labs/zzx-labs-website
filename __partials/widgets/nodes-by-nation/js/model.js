(function(){
  ...
  const W=window;
  if(W.ZZXNodesByNationModel?.__version>=3)return;
  ...
  function meta(code,name="",flag=""){
    ...
    try{if(valid&&typeof Intl?.DisplayNames==="function")label=new Intl.DisplayNames(["en"],{type:"region"}).of(iso)||iso;}catch(_){}
    const glyph=valid?String.fromCodePoint(...[...iso].map(ch=>127397+ch.charCodeAt(0))):"🏴";
    return {code:valid?iso:"--",name:clean(name)||label,flag:clean(flag)||glyph,located:valid};
  }

  function fromNodes(snapshot){
    const counts=new Map();
    for(const node of Array.isArray(snapshot?.nodes)?snapshot.nodes:[]){
      const country=meta(node?.country,node?.countryName,node?.countryFlag);
      if(!country.located||node?.geoSynthetic===true)continue;
      const current=counts.get(country.code)||{...country,nodes:0};
      current.nodes+=1;
  ...
  function fromMap(snapshot){
    ...
    return Object.entries(source).map(([code,count])=>{
      const country=meta(code);
      const nodes=finite(count);
      return country.located&&nodes>0?{...country,nodes}:null;
    }).filter(Boolean);
  }
  ...
  function build(snapshot){
    ...
    for(const row of rows){
      ...
      if(!country.located||!(Number(row.nodes)>0))continue;
      const current=merged.get(country.code)||{...country,nodes:0};
      current.nodes+=Number(row.nodes);
      merged.set(country.code,current);
    }

    rows=[...merged.values()].sort((a,b)=>b.nodes-a.nodes||a.name.localeCompare(b.name)||a.code.localeCompare(b.code));
    ...
    return Object.freeze({
      schema:"zzx-nodes-by-nation-model-v3",
      ...
      geographyJoined:Number(snapshot?.geography?.joined||0),
      top:rows[0]||null
    });
  }

  W.ZZXNodesByNationModel=Object.freeze({__version:3,build,countryMeta:meta});
})();
