// ZZX-Labs / 0xDEADBEEF transaction-atlas theme system.
(function(){
  "use strict";

  const W=window;
  if(W.ZZXMempoolTilesThemes?.__version>=3)return;
  ...
  function apply(root,id){
    ...
    if(root){
      root.dataset.mtTheme=theme.id;
      root.style.setProperty("--mt-bg",colors.background);
      root.style.setProperty("--mt-panel",colors.panel);
      root.style.setProperty("--mt-primary",colors.primary);
      root.style.setProperty("--mt-accent",colors.accent);
      root.style.setProperty("--mt-text",colors.text);
      root.style.setProperty("--mt-muted",colors.muted);
      root.style.setProperty("--mt-border",colors.border);
    }
    return theme;
  }
