(function(){
  ...
  const W=window;
  const D=document;
  if(W.ZZXMempoolTilesInspector?.__version>=3)return;
  ...
  const kv=(label,value)=>`<div class="mt-kv"><dt>${escapeHtml(label)}</dt><dd title="${escapeHtml(value)}">${escapeHtml(value)}</dd></div>`;

  function inputList(rows){
    if(!Array.isArray(rows)||!rows.length)return '<div class="mt-data-row">No inputs.</div>';
    return `<div class="mt-io-list">${rows.map((row,index)=>{
      ...
      return `<div class="mt-io-row">
        <span class="mt-io-index">#${index}</span>
        <span class="mt-io-main"><code>${escapeHtml(source)}</code><br>${escapeHtml(address)}<br>${escapeHtml(prev?.scriptpubkey_type||"unknown")} · sequence ${escapeHtml(text(row?.sequence))} · witness ${witness} item${witness===1?"":"s"}${script?` · scriptSig ${escapeHtml(script)}`:""}</span>
        <span class="mt-io-value">${row?.is_coinbase?"new issuance":sats(prev?.value)}</span>
      </div>`;
    }).join("")}</div>`;
  }

  function outputList(rows,outspends){
    if(!Array.isArray(rows)||!rows.length)return '<div class="mt-data-row">No outputs.</div>';
    return `<div class="mt-io-list">${rows.map((row,index)=>{
      ...
      return `<div class="mt-io-row">
        <span class="mt-io-index">#${index}</span>
        <span class="mt-io-main">${escapeHtml(address)}<br>${escapeHtml(row?.scriptpubkey_type||"unknown")} · script ${escapeHtml(row?.scriptpubkey||"—")}<span class="mt-io-spend">${escapeHtml(state)}${spend?.status?.confirmed?` · confirmed #${escapeHtml(spend.status.block_height)}`:""}</span></span>
        <span class="mt-io-value">${sats(row?.value)}</span>
      </div>`;
    }).join("")}</div>`;
  }
  ...
  function scriptSummary(analysis){
    ...
    return `<div class="mt-data-grid">
      <div class="mt-data-row"><strong>Input script classes</strong><br>${escapeHtml(inputTypes)}</div>
      <div class="mt-data-row"><strong>Output script classes</strong><br>${escapeHtml(outputTypes)}</div>
      <div class="mt-data-row"><strong>Address surface</strong><br>${num(analysis.uniqueInputAddresses,0)} unique input · ${num(analysis.uniqueOutputAddresses,0)} unique output</div>
      <div class="mt-data-row"><strong>Witness surface</strong><br>${num(analysis.witnessItems,0)} stack items · ${num(analysis.witnessBytes,0)} decoded bytes</div>
    </div>`;
  }

  function opReturnList(rows){
    return Array.isArray(rows)&&rows.length
      ? rows.map(row=>`<div class="mt-data-row"><strong>OP_RETURN output #${row.index}</strong><br>UTF-8: ${escapeHtml(row.utf8||"(not printable)")}<br>data hex: ${escapeHtml(row.dataHex||"—")}<br>script: ${escapeHtml(row.asm||row.scriptHex||"—")}</div>`).join("")
  ...
  function blockDetails(analysis){
    ...
    return `<div class="mt-io-list">${rows.map(([label,value])=>`<div class="mt-io-row"><span class="mt-io-index">${escapeHtml(label)}</span><span class="mt-io-main">${escapeHtml(text(value))}</span><span></span></div>`).join("")}</div>`;
  }

  function setState(root,label){
    const node=root.querySelector("[data-mt-inspector-state]");
    if(node)node.textContent=label;
  }

  function render(root,analysis,{persisted=false}={}){
    root.querySelector("[data-mt-inspector-empty]").hidden=true;
    root.querySelector("[data-mt-inspector-body]").hidden=false;
    root.querySelector("[data-mt-inspector-title]").textContent=analysis.txid||"Transaction";
    ...
    root.querySelector("[data-mt-kv-grid]").innerHTML=fields.map(([label,value])=>kv(label,text(value))).join("");
    root.querySelector("[data-mt-inputs]").innerHTML=inputList(analysis.tx?.vin);
    root.querySelector("[data-mt-outputs]").innerHTML=outputList(analysis.tx?.vout,analysis.outspends);
    root.querySelector("[data-mt-scripts]").innerHTML=scriptSummary(analysis);
    root.querySelector("[data-mt-opreturn]").innerHTML=opReturnList(classification.opReturns||[]);
    root.querySelector("[data-mt-block]").innerHTML=blockDetails(analysis);
    root.querySelector("[data-mt-raw-json]").textContent=JSON.stringify(analysis.tx,null,2);
    root.querySelector("[data-mt-raw-hex]").textContent=analysis.rawHex||"raw transaction hex unavailable";
    root.querySelector("[data-mt-input-count]").textContent=`(${num(analysis.inputs,0)})`;
    root.querySelector("[data-mt-output-count]").textContent=`(${num(analysis.outputs,0)})`;
  }
  ...
  function renderSummary(root,record){
    ...
    root.querySelector("[data-mt-inspector-empty]").hidden=true;
    root.querySelector("[data-mt-inspector-body]").hidden=false;
    root.querySelector("[data-mt-inspector-title]").textContent=record.txid||"Transaction";
    ...
    root.querySelector("[data-mt-kv-grid]").innerHTML=fields.map(([label,value])=>kv(label,text(value))).join("");
    for(const selector of ["[data-mt-inputs]","[data-mt-outputs]","[data-mt-scripts]","[data-mt-opreturn]","[data-mt-block]"]){
      root.querySelector(selector).innerHTML='<div class="mt-data-row">Use Revalidate to retrieve the complete transaction record.</div>';
    }
    root.querySelector("[data-mt-raw-json]").textContent="";
    root.querySelector("[data-mt-raw-hex]").textContent="";
    root.querySelector("[data-mt-input-count]").textContent="";
    root.querySelector("[data-mt-output-count]").textContent="";
  }
  ...
  function loading(root,txid){
    ...
    root.querySelector("[data-mt-kv-grid]").innerHTML=kv("Status","loading transaction, raw hex, spend state, block, and Merkle proof…");
    for(const selector of ["[data-mt-inputs]","[data-mt-outputs]","[data-mt-scripts]","[data-mt-opreturn]","[data-mt-block]"]){
  ...
  function error(root,txid,message){
    ...
    root.querySelector("[data-mt-kv-grid]").innerHTML=kv("Status",text(message));
  }
  ...
  function renderReaderList(root,records,activeTxid=""){
    const host=root.querySelector("[data-mt-reader-list]");
    host.replaceChildren();
    for(const record of records){
      const chip=D.createElement("div");
      chip.className=`mt-reader-chip${record.txid===activeTxid?" is-active":""}`;
      chip.tabIndex=0;
      chip.setAttribute("role","button");
      chip.dataset.mtReaderOpen=record.txid;
      ...
      remove.type="button";
      remove.textContent="×";
      remove.dataset.mtReaderRemove=record.txid;
      remove.setAttribute("aria-label",`Unpin ${record.txid}`);
      chip.append(title,detail,remove);
      host.append(chip);
    }
    const count=root.querySelector("[data-mt-reader-count]");
