...
// v2.0.0 — square-only exact-cover shell + live WS + REST fallback
(function(){
  "use strict";
  ...
  const MODULES=[
    ...
    ["ZZXMempoolTilesSorter","js/sorter.js",1],
        ["ZZXMempoolTilesLayout","js/layout.js",4],
    ["ZZXMempoolTilesThemes","js/themes.js",2],
    ["ZZXMempoolTilesRenderer","js/renderer.js",4],
  ...
  async function mount(root,mountedCore=null){
    ...
    W.ZZXMempoolTilesThemes.set(themeId);

    const themeSelect=root.querySelector("[data-mt-theme]");
    if(themeSelect){
      ...
      themeSelect.value=W.ZZXMempoolTilesThemes.get().id;
    ...
    function setActive(){
      ...
      const theme=root.querySelector("[data-mt-theme]");
    ...
    function layoutNow(animate=true){
      ...
      // Exact-cover geometry is drawn atomically. Interpolating square sizes
      // creates transient holes/overlap, violating the Tiles visual contract.
    ...
    function renderStats(){
      ...
      if(gridStat){
        gridStat.textContent=layout.exactCover ? `${fmtInt(layout.leafCount)} square leaves · 100% packed` : `${fmtInt(layout.leafCount)} bootstrap squares`;
      }
      ...
      if(vizCount){
        vizCount.textContent=layout.exactCover ? `${fmtInt(txs)} real TX · square-only · 100% cover` : `${fmtInt(txs)} real TX · bootstrap layout`;
      }
      ...
      if(coverage){
        ...
        coverage.textContent=`${fmtInt(txs)} real candidate TX · ${fill.toFixed(1)}% projected block vsize · ${layout.exactCover?"100% square-packed field":"bootstrap field"}`;
      }
      ...
      if(meta){
        meta.textContent=`real projected-next-block transactions · square-only exact cover · ${scaleMode} scale · ${colorMode} color · ${sortMode} order · ${W.ZZXMempoolTilesThemes.get().name}`;
      }
    }
    ...
    root.querySelector("[data-mt-theme]")?.addEventListener("change",event=>{
