...
(function(){
  ...
  const W=window;
  if(W.ZZXMempoolTilesReaderStore?.__version>=1)return;

  const DB_NAME="zzx-mempool-tiles-readers";
  const DB_VERSION=1;
  const STORE="readers";
  const FALLBACK_KEY="zzx.mempoolTiles.readerSummaries.v1";
  const LAST_KEY="zzx.mempoolTiles.lastReader";
  ...
  async function exportAll(){
    return {
      schema:"zzx-mempool-tiles-reader-export-v1",
      exportedAt:new Date().toISOString(),
      readers:await list(64)
    };
  }

  W.ZZXMempoolTilesReaderStore=Object.freeze({
    __version:1,
    valid,
    tileSnapshot,
    get,
