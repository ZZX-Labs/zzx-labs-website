...
(function(){
  ...
  function packAtN(items,n,{sortMode="priority",seed=0}={}){
    const gridN=Math.max(1,Math.floor(n||1));// __partials/widgets/mempool-tiles/js/packer.js
(function(){
  "use strict";

  const W=window;
  if(W.ZZXMempoolTilesPacker?.__version>=2)return;

  const S=()=>W.ZZXMempoolTilesSorter;

  /*
   * v1 globally sorted by square size after applying the requested arrangement.
   * That made "Likely", "Fee", etc. visually meaningless and produced obvious
   * size bands. v2 preserves arrangement in local windows while still placing
   * larger squares first inside each window to prevent pathological holes.
   */
  function packingOrder(items,sortMode,seed){
    const requested=S().sort(items,sortMode,seed)
      .map((row,index)=>({
        ...row,
        __sortIndex:index
      }));
    ...
    return requested.slice().sort((a,b)=>{
      const ab=Math.floor(a.__sortIndex/windowSize);
      const bb=Math.floor(b.__sortIndex/windowSize);

      if(ab!==bb)return ab-bb;

      const as=Math.max(
        1,
        Math.floor(Number(a.side)||1)
      );

      const bs=Math.max(
        1,
        Math.floor(Number(b.side)||1)
      );
  ...
  function pack(items,{
    ...
    const totalArea=rows.reduce(
      (sum,row)=>
        sum+
        Math.max(1,Number(row.side)||1)**2,
      0
    );

    const maxSide=Math.max(
      ...rows.map(
        row=>Math.max(1,Number(row.side)||1)
      )
    );
    ...
    let n=Math.max(
      ...
      Math.ceil(
        Math.sqrt(
          totalArea/targetFill
        )
      ),
      Math.ceil(
        Math.sqrt(
          rows.length/targetFill
        )
      )
    );
    ...
    for(let i=0;i<64;i++){
      ...
      n=Math.ceil(n*1.04)+1;
    }

    throw new Error(
      `square packing failed: ${result?.rejected?.length||rows.length} unplaced`
    );
  }
