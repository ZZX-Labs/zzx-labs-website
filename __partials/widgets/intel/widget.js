    ...
    start(ctx) {
      ...
      const render = (items) => {
        ...
        if (!out.length) {
          ...
          row.className = "btc-news__item";
          row.innerHTML =
            `<span class="btc-news__src">intel</span><a href="#" tabindex="-1">no items</a>`;
          this.list.appendChild(row);
        }
      };
      ...
      const ghLatest = async (repo) => {
        ...
        const arr = await ctx.fetchJSON(url, {
      ...
      const run = async () => {
        ...
        try {
          ...
          for (const repo of batch) {
            items.push(await ghLatest(repo));
          }
        } catch {}

        // Hacker News
        try {
          const data = await ctx.fetchJSON(ctx.api.HN_QUERY);
