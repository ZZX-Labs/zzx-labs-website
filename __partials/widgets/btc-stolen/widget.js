(function () {
  ...
  window.ZZXWidgetRegistry.register(ID, {
    ...
    async load() {
      ...
      const url = this._core.join(prefix, `/__partials/widgets/btc-stolen/btc-stolen.json`);
      try {
    ...
    render() {
      ...
      if (!slice.length) {
        list.innerHTML = `<div class="row"><span class="k">case</span><span class="v">no data</span></div>`;
      }
