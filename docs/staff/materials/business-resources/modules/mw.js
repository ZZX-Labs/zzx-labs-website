...
// Fallbacks if config.js is older
...
async function fetchWithRetryJSON(url, { retries = RETRIES, timeout = REQ_TO_MS, init = FETCH_INIT } = {}) {
  ...
  while (attempt <= retries) {
    try {
      return await fetchJSONWithTimeout(url, { timeout, init });
    } catch (e) {
      lastErr = e;
      if (attempt === retries) break;
      await sleep(BACKOFF * Math.pow(2, attempt)); // simple exponential backoff
      attempt++;
    }
  }
  throw lastErr;
...
async function fetchParse(params) {
  ...
  // If MW returned an API-level error, surface it
  if (json && json.error) {
...
export async function resolveAndInspect(rawTitle) {
  ...
  if (!first) {
    throw new Error('MediaWiki response malformed (no pages)');
  }
