...
def write_json(path: Path, payload: Any, pretty: bool = True) -> None:
    ...
    Crawler state is long-lived. A power loss or service restart must never leave
    a half-written nodes/queue/meta file that destroys the next cycle.
    """
    path = Path(path)
    mkdir(path.parent)
...
def write_gzip_json(path: Path, payload: Any, pretty: bool = False) -> None:
    """Atomically replace a gzip-compressed JSON file."""
    path = Path(path)
    mkdir(path.parent)
    tmp = path.parent / f".{path.name}.{os.getpid()}.{time.time_ns()}.tmp"
    try:
...
class BitnodesState:
    ...
    def all_candidate_addresses(
        ...
        **_kwargs: Any,
        ...
        """Return the complete deduplicated candidate universe.

        This method deliberately does not implement crawl ordering; use
        ``select_probe_candidates`` for a bounded probe batch. Keeping the two
        responsibilities separate prevents an alphabetical truncation from
        starving addresses late in the key space.
        """
        candidates: list[str] = []
    ...
    def select_probe_candidates(
        ...
        *,
        ...
        batch_size: int = 0,
        now: int | None = None,
        reachable_interval_seconds: int = 300,
        unreachable_interval_seconds: int = 900,
    ) -> list[str]:
        """Select a fair, due-first probe batch from every known address.

        New addresses are probed first. Existing addresses are ordered by their
        oldest successful/failed probe time and are eligible again once their
        reachability-specific interval expires. If the due set is smaller than
        the requested batch, the oldest not-yet-due addresses fill the batch so
        every cycle remains productive without repeatedly selecting the same
        alphabetically first nodes.
        """
        ts = int(now or utc_now())
        # Rank the complete candidate universe before applying ``limit``.
        # Truncating the alphabetically normalized universe first would make
        # addresses outside that prefix permanently ineligible when the known
        # set exceeds the configured scheduler window.  Applying the limit to
        # the age/due-ranked list instead lets every address rotate into the
        # active window as earlier selections receive fresh probe timestamps.
        universe = self.all_candidate_addresses(
        ...
        seed_set = set(self._normalize_addresses(seed_addresses or []))
        ranked: list[tuple[int, int, str]] = []

        for address in universe:
            record = self.nodes.get(address) or {}
            metadata = normalize_metadata(record.get("metadata"))
            last_success = to_int(record.get("last_success") or metadata.get("last_success"), 0) or 0
            last_failure = to_int(record.get("last_failure") or metadata.get("last_failure"), 0) or 0
            last_probe = max(last_success, last_failure)
            reachable_now = boolish(record.get("reachable_now") if record.get("reachable_now") is not None else metadata.get("reachable_now"))
            interval = max(0, int(reachable_interval_seconds if reachable_now else unreachable_interval_seconds))

            # priority 0: never-probed/newly discovered; 1: due; 2: oldest fill
            if not record or last_probe <= 0:
                priority = 0
            elif ts - last_probe >= interval:
                priority = 1
            else:
                priority = 2

            # Newly returned DNS/getaddr seeds receive preference inside the
            # same priority without defeating age-based fairness.
            seed_bias = 0 if address in seed_set else 1
            ranked.append((priority * 2 + seed_bias, last_probe, address))
        ...
        target = len(ranked) if int(batch_size or 0) <= 0 else min(int(batch_size), len(ranked))
        if target <= 0:
            return []

        selected = [address for _, _, address in ranked[:target]]

        self.meta["probe_scheduler"] = {
            "updated_at": utc_iso(ts),
            "universe": len(universe),
            "scheduler_window": len(ranked),
            "selected": len(selected),
            "reachable_interval_seconds": int(reachable_interval_seconds),
            "unreachable_interval_seconds": int(unreachable_interval_seconds),
        }
        return selected
    ...
    def update_failures(
        ...
        **_kwargs: Any,
        ...
        for address, failure_info in failure_items:
            normalized = normalize_address(address=address)
            ...
            if isinstance(failure_info, Mapping) and failure_info.get("error"):
                metadata["last_error"] = str(failure_info.get("error"))[:2048]
    ...
    def to_bitnodes_nodes(self, mode: str = "all") -> dict[str, list[Any]]:
        ...
        for address, record in self.nodes.items():
            ...
            reachable = boolish(metadata.get("reachable") if metadata.get("reachable") is not None else record.get("reachable"))
            reachable_now = boolish(metadata.get("reachable_now") if metadata.get("reachable_now") is not None else record.get("reachable_now"))
            last_success = to_int(record.get("last_success") or metadata.get("last_success"), 0) or 0
            reachable_24h = bool(last_success and now - last_success <= 86400)
            reachable_week = bool(last_success and now - last_success <= 7 * 86400)
            reachable_month = bool(last_success and now - last_success <= 31 * 86400)

            # Window flags are derived from timestamps at export time. Persisted
            # booleans are historical hints only and may otherwise stay true
            # forever after a node disappears.
            metadata["reachable"] = reachable
            metadata["reachable_now"] = reachable_now
            metadata["reachable_24h"] = reachable_24h
            metadata["reachable_week"] = reachable_week
            metadata["reachable_month"] = reachable_month
            row[19] = metadata
    ...
    def snapshot_24h(self, payload: dict[str, Any] | None = None) -> Path:
        """Write one compressed rolling-24h snapshot and prune expired local copies.

        Indefinite history belongs in ``history_archive.py`` / the private
        registry. This directory is intentionally a bounded operational window.
        """
        payload = payload or self.build_export_payload("reachable_24h")
        timestamp = int(payload.get("timestamp") or utc_now())
        path = self.snapshot_24h_dir / (time.strftime("%Y%m%dT%H%M%SZ", time.gmtime(timestamp)) + ".json.gz")
        write_gzip_json(path, payload)

        cutoff = timestamp - (25 * 3600)
        for candidate in self.snapshot_24h_dir.glob("*.json*"):
            if candidate == path:
                continue
            try:
    ...
    def state_summary(self) -> dict[str, Any]:
        ...
        for address, record in self.nodes.items():
            ...
            is_reachable_now = boolish(record.get("reachable_now") if record.get("reachable_now") is not None else metadata.get("reachable_now"))
            last_success = to_int(record.get("last_success") or metadata.get("last_success"), 0) or 0
            is_reachable_24h = bool(last_success and now - last_success <= 86400)
            is_reachable = boolish(record.get("reachable") if record.get("reachable") is not None else metadata.get("reachable"))
