...
def write_json_file(path: Path, payload: Any, pretty: bool = True) -> None:
    """Atomically publish JSON so readers never observe a partial crawler file."""
    mkdir(path.parent)
...
def write_gzip_json_file(path: Path, payload: Any, pretty: bool = False) -> None:
    """Atomically publish gzip JSON used by bounded runtime pointers."""
    mkdir(path.parent)
    tmp = path.parent / f".{path.name}.{os.getpid()}.{time.time_ns()}.tmp"
    try:
...
def write_bucket_snapshots(source: str, payload: dict[str, Any], pretty: bool = True) -> None:
    ...
    pointer = {
        "schema": "zzx-bitnodes-snapshot-pointer-v4",
        "source": source,
        ...
        "policy": "Public bucket pointers are bounded; immutable full snapshots and raw observations are retained in the private history archive and MariaDB shards.",
    }
    ...
    for bucket, path in dirs.items():
        index = {
            "schema": "zzx-bitnodes-snapshot-bucket-index-v4",
            "source": source,
            "bucket": bucket,
            "generated_at": iso_now(),
    ...
    write_json_file(
        SNAPSHOTS_ROOT / source / "manifest.json",
        {
            "schema": "zzx-bitnodes-snapshot-buckets-v4",
            "source": source,
            "generated_at": iso_now(),
            "policy": pointer["policy"],
            "latest": pointer,
...
def discover_from_existing_files(
    ...
    for path in collect_seed_files(
...
def discover_dns(limit: int, timeout: float = 5.0) -> list[str]:
    ...
    return [
        normalized
        for host in sorted(set(discovered))[:limit]
...
def expand_getaddr(
    ...
    for round_index in range(rounds):
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(getaddr_from_node, address, timeout) for address in batch]
...
def crawl_address(address: str, timeout: float) -> tuple[str, list[Any] | None, dict[str, Any] | None]:
    """Probe one address and preserve success or failure telemetry.

    Failed probes are data too: the private observation stream keeps the error,
    transport class, endpoint and measured attempt latency instead of reducing a
    failed connection to an address-only boolean.
    """
    requested = normalize_address(address) or str(address)
    started = time.perf_counter()
    try:
        info = handshake(address, timeout=timeout)
        latency_ms = round((time.perf_counter() - started) * 1000.0, 2)
        normalized = normalize_address(info.address) or requested

        if not info.connected:
            return normalized, None, {
                "error": info.error or "handshake did not complete",
        ...
        metadata["is_cjdns"] = info.network == "cjdns"
        metadata["last_seen"] = utc_now()
        metadata["crawler"] = SOURCE
        metadata["source"] = SOURCE
        metadata["crawler_version"] = "zzxbitnodes-enhanced-v7"
        metadata["crawl_observed_at"] = iso_now()
        row[19] = metadata
        return normalized, row, None
...
def crawl_batch(addresses: list[str], timeout: float, workers: int) -> tuple[dict[str, list[Any]], dict[str, dict[str, Any]]]:
    successes: dict[str, list[Any]] = {}
    failures: dict[str, dict[str, Any]] = {}

    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_map = {executor.submit(crawl_address, address, timeout): address for address in addresses}
        for future in as_completed(future_map):
            requested_address = future_map[future]
            try:
                address, row, failure = future.result()
            except Exception as exc:
                address = normalize_address(requested_address) or str(requested_address)
                row = None
                failure = {"error": f"{type(exc).__name__}: {exc}", "observed_at": iso_now()}
            if row is not None:
                successes[address] = row
            else:
                failures[address] = failure or {"error": "unknown probe failure", "observed_at": iso_now()}

    return successes, failures


def build_changes(state_before: dict[str, dict[str, Any]], state_after: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": "zzx-bitnodes-change-set-v4",
        "generated_at": iso_now(),
...
def geoip_available(geoip_enabled: bool, city_db: Path, asn_db: Path, country_db: Path) -> bool:
    ...
    printf("[geoip] GeoIP enabled, but local mmdb files are incomplete. Missing: " + ", ".join(missing))
    return False


def enrich_state_records(state: BitnodesState, geoip_enabled: bool, city_db: Path, asn_db: Path, country_db: Path) -> None:
...
def export_state_direct(
    ...
    archive_pointer = {
        "schema": "zzx-bitnodes-runtime-archive-pointer-v2",
        "source": SOURCE,
        "generated_at": iso_now(),
        "timestamp": timestamp,
        ...
        "reachable_nodes": payload.get("reachable_nodes"),
        "latest_height": payload.get("latest_height"),
        "policy": "Public runtime archive is bounded; immutable full snapshots and probe observations are retained in the private history archive and MariaDB shards.",
    }

    write_json_file(archive_dir / archive_name, archive_pointer, pretty=pretty)

    gzip_path = archive_dir / f"{archive_name}.gz"
    write_gzip_json_file(gzip_path, archive_pointer, pretty=pretty)
...
def run_export_wrapper(*, input_path: Path, compact: bool) -> int:
...
def run_ipdb(*, input_path: Path, output_path: Path, ipdb_dir: Path, max_segment_bytes: int, compact: bool) -> int:
    if not IP_DB.exists():
        return 0
...
def run_push_ipdb(*, source_dir: Path, compact: bool) -> int:
...
def run_registry_backup(*, input_dir: Path, api_dir: Path, output_dir: Path, latest_dir: Path, enabled: bool) -> int:
    if not enabled:
        return 0
...
def run_registry_index(*, registry_root: Path, enabled: bool) -> int:
    ...
    command = py(UPDATE_DAILY_INDEX, "--repo-root", str(registry_root))
    return run_command(command).returncode


def run_maps_after(*, latest_input: Path, api_dir: Path, state_dir: Path, enabled: bool, compact: bool) -> int:
    if not enabled:
        return 0
...
def push_snapshots(enabled: bool, message: str) -> int:
    ...
    command = py(
        ...
        "bitcoin/bitnodes/data/geoip/current",
        "bitcoin/bitnodes/data/geoip/published",
...
def crawl_once(
    *,
    ...
    git_push: bool,
    recrawl_reachable_seconds: int = 300,
    recrawl_unreachable_seconds: int = 900,
    history_full_snapshot_interval_seconds: int = 900,
    ...
    state = BitnodesState(state_dir=state_dir, snapshot_24h_dir=snapshot_24h_dir, source=SOURCE)
    before = json.loads(json.dumps(state.nodes, default=str))
    ...
    candidates = state.select_probe_candidates(
        seed_addresses=expanded_seed_addresses + discovered,
        limit=limit,
        batch_size=batch_size,
        now=now,
        reachable_interval_seconds=recrawl_reachable_seconds,
        unreachable_interval_seconds=recrawl_unreachable_seconds,
    )

    successes, failures = crawl_batch(addresses=candidates, timeout=timeout, workers=workers)

    state.update_successes(successes, now=now)
    state.update_failures(failures, now=now)

    enrich_state_records(state, geoip_enabled=geoip_enabled, city_db=city_db, asn_db=asn_db, country_db=country_db)

    state.meta.update(
        {
            "crawler": SOURCE,
            "source": SOURCE,
            "crawler_version": "zzxbitnodes-enhanced-v7",
            "last_crawl": now,
            ...
            "recrawl_reachable_seconds": int(recrawl_reachable_seconds),
            "recrawl_unreachable_seconds": int(recrawl_unreachable_seconds),
            "history_full_snapshot_interval_seconds": int(history_full_snapshot_interval_seconds),
    ...
    if history_output_dir is not None:
        # Private history is intentionally all-known-state, not the bounded
        # reachable_24h public interchange view. This preserves every node the
        # crawler has learned about, including stale/unreachable addresses and
        # their accumulated metadata, while keeping the public repository small.
        history_payload = state.build_export_payload(mode="all")
        history_payload["source"] = SOURCE
        history_payload["crawler"] = SOURCE
        history_payload["crawler_version"] = "zzxbitnodes-enhanced-v7"
        history_payload["changes"] = changes
        history_payload["generated_at"] = iso_now()
        manifest = record_cycle(
            history_output_dir,
            source=SOURCE,
            timestamp=now,
            ...
            failures=failures,
            changes=changes,
            full_snapshot_interval_seconds=history_full_snapshot_interval_seconds,
        )
        printf(
            f"[history] cycle={manifest['cycle_id']} "
            f"nodes={manifest['nodes']} successes={manifest['successes']} failures={manifest['failures']}"
        )
    ...
    if mirror_legacy:
        mirror_latest_to_legacy_api(source_dir=output_dir, legacy_dir=DEFAULT_LEGACY_API, pretty=pretty)
    ...
    code = push_snapshots(enabled=git_push, message="Update ZZX Bitnodes global node dataplane snapshots")
...
def add_argument_if_missing(parser: argparse.ArgumentParser, *flags: str, **kwargs: Any) -> None:
...
def build_parser(description: str = "ZZX-Labs persistent enhanced Bitnodes global crawler.") -> argparse.ArgumentParser:
    ...
    add_argument_if_missing(parser, "--history-output-dir", default="")
    ...
    add_argument_if_missing(parser, "--profile", choices=["github", "local", "aggressive"], default="github")
    ...
    add_argument_if_missing(parser, "--recrawl-reachable-seconds", type=int, default=300)
    add_argument_if_missing(parser, "--recrawl-unreachable-seconds", type=int, default=900)
    add_argument_if_missing(parser, "--history-full-snapshot-interval-seconds", type=int, default=900)
    ...
    add_argument_if_missing(parser, "--max-segment-bytes", type=int, default=DEFAULT_MAX_PUBLIC_JSON_BYTES)
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    defaults = {
        ...
        "getaddr_rounds": 16,
        "dns_seed_limit": 4096,
        "recrawl_reachable_seconds": 300,
        "recrawl_unreachable_seconds": 900,
        "history_full_snapshot_interval_seconds": 900,
...
def run_from_args(args: argparse.Namespace) -> int:
    ...
    common = {
        ...
        "recrawl_reachable_seconds": max(0, int(args.recrawl_reachable_seconds)),
        "recrawl_unreachable_seconds": max(0, int(args.recrawl_unreachable_seconds)),
        "history_full_snapshot_interval_seconds": max(0, int(args.history_full_snapshot_interval_seconds)),
        ...
        "history_output_dir": Path(args.history_output_dir) if args.history_output_dir else None,
        "strict": bool(args.strict),
    }

    if args.daemon:
        daemon_loop(**common, interval=int(args.interval), run_seconds=int(args.run_seconds))
        return 0
