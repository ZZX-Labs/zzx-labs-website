...
DEFAULT_ZZX_STATE_DIR = BITNODES_ROOT / "data" / "state" / "zzxbitnodes"
DEFAULT_ZZX_SNAPSHOT_24H_DIR = BITNODES_ROOT / "data" / "snapshots" / "24h" / "zzxbitnodes"
    ...
    if isinstance(payload, dict):
        ...
        for value in payload.values():
            if isinstance(value, dict):
                found.extend(
                    extract_nodes_from_payload(value)
                )
        ...
        for item in payload:
            if isinstance(item, str):
                ...
                found.extend(
                    extract_nodes_from_payload(item)
                )
...
def mirror_latest_to_legacy_api(
    source_dir: Path,
    legacy_dir: Path,
    pretty: bool = True
) -> None:
...
def crawl_once(
    ...
    state.meta["crawler"] = "zzxbitnodes"
    state.meta["source"] = "zzxbitnodes"
...
def build_parser(
    description: str = "ZZX-Labs persistent Bitnodes global crawler."
    ...
    parser.add_argument(
        "--state-dir",
        default=str(DEFAULT_ZZX_STATE_DIR)
    )

    parser.add_argument(
        "--snapshot-24h-dir",
        default=str(DEFAULT_ZZX_SNAPSHOT_24H_DIR)
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also mirror zzxbitnodes/latest.json into bitcoin/bitnodes/api/latest.json "
            "for old frontend compatibility."
        )
    )
