...
def json_value_at_commit(commit: str, path: str, field: str):
    cp = git("show", f"{commit}:{path}", check=False, capture=True)
...
def remote_is_newer_or_equal(payload: str, remote: str, path: str, field: str) -> bool:
    proposed = json_value_at_commit(payload, path, field)
    current = json_value_at_commit(remote, path, field)
...
def main() -> int:
    ...
    ap.add_argument("--monotonic-json-path", default="")
    ap.add_argument("--monotonic-field", default="timestamp")
    args = ap.parse_args()
    ...
    for attempt in range(1, args.attempts + 1):
        ...
        fetch = git("fetch", args.remote, args.branch, check=False)
        if fetch.returncode != 0:
            print(
                f"publication fetch attempt {attempt}/{args.attempts} failed; retrying without force",
                file=sys.stderr,
                flush=True,
            )
        ...
        if args.monotonic_json_path and remote_is_newer_or_equal(
            payload, remote_head, args.monotonic_json_path, args.monotonic_field
        ):
            print(
                "remote monotonic state is newer than or equal to this payload; "
                "skipping stale generated publication",
                flush=True,
            )
            return 0
