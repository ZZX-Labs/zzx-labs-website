...
RUN_DIR = Path(os.environ.get("ZZX_LABS_RUN_DIR", str(APP_ROOT / "run")))
LOG_DIR = Path(os.environ.get("ZZX_LABS_LOG_DIR", str(APP_ROOT / "log" / "bitnodes")))
...
def build_crawl_command(config: dict[str, Any], daemon_cycle: bool = False) -> list[str]:
    ...
    interval = int(crawler_cfg.get("crawl_interval_seconds", 5))
    recrawl_reachable = int(crawler_cfg.get("recrawl_reachable_seconds", 300))
    recrawl_unreachable = int(crawler_cfg.get("recrawl_unreachable_seconds", 900))
    history_full_snapshot_interval = int(crawler_cfg.get("history_full_snapshot_interval_seconds", 900))
    export_mode = str(export_cfg.get("mode", "reachable_24h"))
    history_output_dir = Path(
        export_cfg.get(
            "history_output_dir",
            os.environ.get("ZZX_BITNODES_HISTORY_DIR", "/var/lib/zzx-bitnodes/history"),
        )
    )

    api_dir = Path(export_cfg.get("base_dir", str(DEFAULT_API_DIR)))
    crawler_output = Path(export_cfg.get("crawler_output", str(api_dir / "zzxbitnodes")))
    archive_dir = Path(export_cfg.get("archive_dir", str(DEFAULT_ARCHIVE_DIR / "runtime" / "zzxbitnodes")))
    ...
    cmd = [
        ...
        export_mode,
        "--history-output-dir",
        str(history_output_dir),
        "--recrawl-reachable-seconds",
        str(max(0, recrawl_reachable)),
        "--recrawl-unreachable-seconds",
        str(max(0, recrawl_unreachable)),
        "--history-full-snapshot-interval-seconds",
        str(max(0, history_full_snapshot_interval)),
    ]

    if not bool(geoip_cfg.get("enabled", True)):
        cmd.append("--disable-geoip")

    # Resident mode must remain acquisition-focused. Expensive enrichment,
    # aggregation, database publication, IPDB generation, and maps are handled
    # by independent snapshot/publication timers so they can never stall the
    # next P2P probe cycle.
    if daemon_cycle:
        cmd.extend([
            "--no-enrich-after",
            "--no-geo-contract-after",
            "--no-aggregate-after",
            "--no-export-all-after",
            "--no-ipdb-after",
        ])
...
def run_cycle(config: dict[str, Any], daemon_cycle: bool = False) -> int:
    ...
    if daemon_cycle:
        # The resident service is an acquisition process only. Enrichment,
        # geodata, maps and publication are deliberately delegated to the
        # snapshot timer so presentation work can never delay the next probe.
        write_status(
            config,
...
def daemon_loop() -> int:
    ...
    config = load_config()
    interval = int(cfg_get(config, ["crawler", "crawl_interval_seconds"], 5))
...
def write_config_example() -> int:
    payload = {
        ...
        "export": {
            ...
            "crawler_output": str(DEFAULT_API_DIR / "zzxbitnodes"),
            "archive_dir": "/var/lib/zzx-bitnodes/archive/zzxbitnodes",
            "state_dir": "/var/lib/zzx-bitnodes/state/zzxbitnodes",
            "snapshot_24h_dir": "/var/lib/zzx-bitnodes/snapshots/24h/zzxbitnodes",
            "compact": False,
            "history_output_dir": os.environ.get("ZZX_BITNODES_HISTORY_DIR", "/var/lib/zzx-bitnodes/history"),
        },
        "geoip": {
            "enabled": True,
        },
