...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="zzx-publish-overlay-") as td:
        ...
        # A stale overlapping crawler must never roll public latest backward.
        git(racer, "fetch", "origin", "main")
        git(racer, "reset", "--hard", "origin/main")
        write(racer, "bitcoin/bitnodes/api/zzxbitnodes/latest.json", '{"timestamp":200,"nodes":{}}\n')
        git(racer, "add", "bitcoin/bitnodes/api/zzxbitnodes/latest.json")
        git(racer, "commit", "-m", "newer live snapshot")
        git(racer, "push", "origin", "main")

        git(runner, "fetch", "origin", "main")
        git(runner, "reset", "--hard", "origin/main")
        write(runner, "bitcoin/bitnodes/api/zzxbitnodes/latest.json", '{"timestamp":100,"nodes":{}}\n')
        write(runner, "bitcoin/bitnodes/api/index.json", '{"generation":"stale"}\n')
        git(runner, "add", "bitcoin/bitnodes/api")
        git(runner, "commit", "-m", "stale payload")
        stale_payload = git(runner, "rev-parse", "HEAD")

        stale = run([
            sys.executable, str(SCRIPT),
            "--payload-commit", stale_payload,
            "--remote", "origin", "--branch", "main",
            "--message", "stale publish", "--attempts", "1", "--sleep-seconds", "0",
            "--allowed-prefix", "bitcoin/bitnodes/api",
            "--monotonic-json-path", "bitcoin/bitnodes/api/zzxbitnodes/latest.json",
            "--monotonic-field", "timestamp", "--clean",
        ], runner)
        git(runner, "fetch", "origin", "main")
        live = git(runner, "show", "origin/main:bitcoin/bitnodes/api/zzxbitnodes/latest.json")
        if '"timestamp":200' not in live:
            raise AssertionError("monotonic publisher allowed stale latest rollback")
        if "skipping stale generated publication" not in stale.stdout:
            raise AssertionError("monotonic stale publication was not diagnosed")
