// /music/script.js — wrapper: wiring + state (SomaFM-only meta via AllOrigins)
import { repoPrefix, clamp01, fmtTime, corsWrap } from './modules/utils.js';
import { loadM3U } from './modules/m3u.js';
import { fetchTrackMeta, fetchStreamMeta } from './modules/metadata.js'; // NOTE: fetchStreamMeta added
import { ensureMeter } from './modules/meter.js';
import {
  buildShell, setNow, renderPlaylistList, renderRadioList,
  updateRadioNow, updateRadioListeners, highlightList
} from './modules/ui.js';

const root = document.querySelector('[data-mp]');
if (!root) { console.error('[music] no [data-mp] element'); }

// ---------- tiny AllOrigins helpers (front-end only) ----------
const AO_BASE = 'https://api.allorigins.win';
const ao = (url, mode='raw') => `${AO_BASE}/${mode==='json'?'get':'raw'}?url=${encodeURIComponent(url)}&disableCache=true`;
const wrapProxy = (proxy, url) => {
  if (!proxy) return url;
  const p = String(proxy).toLowerCase();
  if (p.startsWith('allorigins')) return ao(url, p.includes('json')?'json':'raw');
  return corsWrap(proxy, url);
};
async function fetchJSONViaProxy(url, proxy){
  const p = String(proxy||'').toLowerCase();
  if (p.startsWith('allorigins')){
    try { const r = await fetch(ao(url,'raw'), {cache:'no-store'}); if (r.ok) return await r.json(); } catch {}
    try {
      const r = await fetch(ao(url,'json'), {cache:'no-store'}); if (!r.ok) return null;
      const j = await r.json(); const txt = j?.contents || ''; if (!txt) return null;
      try { return JSON.parse(txt); } catch { return null; }
    } catch {}
    return null;
  }
  try { const r = await fetch(wrapProxy(proxy, url), {cache:'no-store'}); return r.ok ? await r.json() : null; } catch { return null; }
}
...
const pushHistory = (title, item)=>{
  const k = stationKey(title, lastStreamUrl);
  const arr = historyByStation.get(k) || [];
  if (!arr.length || arr[arr.length-1] !== item) { arr.push(item); if (arr.length>250) arr.shift(); historyByStation.set(k, arr); }
};
...
function wireControls(refs){
  refs.switchKnob?.addEventListener('click', async ()=>{
    ...
    setSwitch(refs, isRadio); // flip
    if (usingStations) await onPickStations(refs, true);
    else               await onPickMusic(refs, true);
  });
  ...
  if (refs.vol){
    ...
    refs.vol.addEventListener('input', ()=>{
      audio.volume = clamp01(parseFloat(refs.vol.value));
    });
  }
...
async function onPickStations(refs, autoPlay){
  ...
  // IMPORTANT: pass proxy for cross-origin PLS
  queue = await loadM3U({ path:file, base, audioBase:cfg.audioBase, isStation:true,
                          selectedTitle: refs.sel.stations?.selectedOptions?.[0]?.textContent,
                          proxy: cfg.corsProxy });
  cursor = 0;
  lastNowTitle = ''; // ensure first poll paints
  const stTitle = queue[0]?.title || 'Live Station';
  renderRadioList(refs, stTitle, '—', stationHistory(stTitle));
...
async function pollOnce(refs, stationTitle){
  try{
    // Only use SomaFM channels.json (no ICY):
    const meta = await fetchStreamMeta(lastStreamUrl, cfg.corsProxy, { name: stationTitle });
    if (!meta) return;

    // listeners → "123 • LIVE"
    if (meta.listeners != null) updateRadioListeners(refs, meta.listeners);
