// /music/script.js — DROP-IN (SomaFM-only now-playing via channels.json through proxy)
// - Renders player UI
// - Loads manifest + playlists/stations
// - Plays streams/files
// - Polls ONLY SomaFM channels.json for now-playing + listeners
// - Shows listeners to the left of “LIVE”, and lastPlaying as the row beneath

import { repoPrefix, clamp01, fmtTime } from './modules/utils.js';
import { loadM3U } from './modules/m3u.js';
import { fetchStreamMeta, fetchTrackMeta } from './modules/metadata.js'; // SomaFM-only for streams; simple parse for files
import {
  buildShell,
  setNow,
  renderPlaylistList,
...
const cfg = (() => {
  ...
  return {
    ...
    corsProxy     : (attr('data-cors-proxy') || 'allorigins-raw').trim(), // works with our cors.js helpers
    metaPollSec   : 8
  };
})();
...
(async function boot(){
  ...
  const mf = await (async ()=>{
    try{ const r = await fetch(cfg.manifestUrl, { cache:'no-store' }); return r.ok ? await r.json() : {}; }
    catch { return {}; }
  })();
  ...
  // Polite autoplay if muted requested but play() was blocked
...
async function onPickStations(refs, autoPlay){
  ...
  renderRadioList(refs, stTitle, '—', stationHistory(stTitle), lastListeners);
...
async function playAt(refs, i){
  ...
  try{
    if (tr.isStream){
      ...
      renderRadioList(refs, tr.title, lastNowTitle || '—', stationHistory(tr.title), lastListeners);
      ...
      // One-shot metadata for file tracks
      try {
...
function nextTrack(refs){
  ...
  if (cfg.shuffle){
    ...
    const n = cursor + 1;
    if (n >= queue.length){
      if (loopMode==='all') return playAt(refs, 0);
      setPlayIcon(refs, false);
    } else playAt(refs, n);
  }
}
...
async function pollOnce(refs, stationTitle){
  try {
    // IMPORTANT: SomaFM-only — uses channels.json via /music/modules/metadata.js
    const meta = await fetchStreamMeta(lastStreamUrl, cfg.corsProxy, { name: stationTitle });
    if (!meta) return;

    // Listeners • LIVE (row 0, right side)
